...
if ($result->num_rows > 0) {
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    $sql1 = "SELECT * FROM usuario WHERE IdUsuario ='$edit_id'";
...
<body>
  ...
  <main>
    <div class="section-1 mt-5">
      ...
      <div>
        <h3 class=' title-adm2'>Painel de Usuários</h3>
        <p class=' title-adm2 pb-3'>Local de listagem e gerenciamento de usuários.</p>
      </div>

      <div class='caixa__login_edit p-4 border-opacity-50 border border-black'>

        <a href='../index.php?e=c4ca4238a0b923820dcc509a6f75849b' class='x-sair'>X</a>

        <h3 class='text-center text-title pb-4'>Editando Conta </h3>

        <form class='row' action='../../scripts/edit_user.php' method='POST' enctype='multipart/form-data'>
          ...
          <div class="col-5">
            <?php if (isset($foto2)) { ?>
              <img id="preview" height="200" src="../../fotos/usuarios/<?php echo $foto2; ?>" class="pb-2" style="border-radius: 30px; min-width: 200px; max-width: 300px;">
            <?php } else { ?>
              <img id="preview" height="200" src="../../fotos/outros/user.png" class="pb-2" style="border-radius: 30px; min-width: 200px; max-width: 300px;">
            <?php } ?>
            <br>
            <input class='form-control border-1 border-dark' accept="image/*" type="file" name="imagem" id="imagem" onchange="previewImagem(event)" value="<?php echo $foto2; ?>">
          </div>

          <div class="col-2"></div>

          <button type='submit' class='btn border border-black border-opacity-25 ml-3' style='background-color:#80CCB1; width: 120px'>Editar</button>
        </form>
      </div>
